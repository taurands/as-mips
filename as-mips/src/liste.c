...
/**
 * @param liste_pp Pointeur de pointeur de liste à créer
 * @param freeFn Pointeur sur la fonction de destruction des données dynamiques liées à l'élement de liste
 * @return SUCCESS si la liste a été crée, FAIL_ALLOC s'il n'y a pas assez de mémoire, FAILURE si une liste est déjà pointée ou si le pointeur de pointeur de liste est nul.
...
/**
 * @param liste_pp pointeur sur un pointeur de liste générique simplement chaînée
 * @return SUCCESS si la liste à détruire existe et a bien été effacée, FAILURE sinon
